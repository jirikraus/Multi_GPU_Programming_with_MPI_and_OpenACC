...
int main(int argc, char** argv)
{
    ...
    acc_device_t used_device_type;
    if ( rank < ngpus )
    {
        used_device_type = acc_device_nvidia;
        acc_set_device_type(used_device_type);
        int devicenum=rank%ngpus;
        acc_set_device_num(devicenum,used_device_type);
    }
    else
    {
        used_device_type = acc_device_host;
        acc_set_device_type(used_device_type);
    }
    // Call acc_init after acc_set_device_num to avoid multiple contexts on device 0 in multi GPU systems
    acc_init(used_device_type);
#endif /*_OPENACC*/

    float cpu_gpu_ratio = 0.10f;
    
    int N_CPU = ( (cpu_gpu_ratio * max( (size-ngpus), 0 ) ) / ngpus ) * N;
    int N_GPU = N - N_CPU;
    int cpu_chunk_size = ceil( 1.0*N_CPU/(size-ngpus) );
    int gpu_chunk_size = ceil( 1.0*N_GPU/min( ngpus, size ) );

    int jstart;
    int jend;
    
    if ( used_device_type == acc_device_nvidia )
    {
        jstart = rank * gpu_chunk_size;
        jend   = min( jstart + gpu_chunk_size, N_GPU );
    }
    else
    {
        jstart = N_GPU + (rank-ngpus) * cpu_chunk_size;
        jend   = jstart + cpu_chunk_size;
    }
